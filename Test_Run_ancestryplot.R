## Test_Run_ancestryplot.R for KING Ancestry plot, by Zhennan Zhu and Wei-Min Chen
library(e1071)
prefix="Test_Run"
...
pred.out <- cbind(test.data[, c("FID", "IID", "PC1", "PC2")], pred_class, pred_prob, orders.class[, 1], orders.class[, 2], round(orders.probs[, 1], 3), round(orders.probs[, 2], 3))
...
if(!require(ggplot2, quietly=TRUE)) {
  plot(pred.out$PC1, pred.out$PC2, col = pred.colors, xlab = "PC1", ylab = "PC2", main=paste("Inferred Populations as Ancestry in", prefix), pch = 16)
